...
def ELBO(x_data, y_data, low_bdd, X_M, phi, sigma, alpha_q, beta_q, lamda_p):
    ...
    return -result
def acq_fcn_UCB(xx,x_data, y_data, low_bdd, X_M, phi, sigma, q_mu):
...
for iter in range(50):
    ...
    acq_fcn_val = acq_fcn_UCB(tf.sigmoid(pt_sel), x_data_p, y_data_p, low_bdd_p, X_M, phi_p, sigma_p,
                              tf.exp(alpha_v - beta_v))
