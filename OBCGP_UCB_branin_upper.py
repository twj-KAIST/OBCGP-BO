...
def ELBO(x_data,y_data,low_bdd,upp_bdd,X_M,phi,sigma,alpha_q,beta_q,lamda_p):
    ...
    return -result
def acq_fcn_UCB(xx,x_data, y_data, low_bdd,upp_bdd, X_M, phi, sigma, q_mu):
    ...
    OBCGP_var = tf.nn.relu(GP_var_star - tf.square(ratio_up / (ratio_down + eps)) * (tf.exp(sigma) * ratio_down))

    fcn_val=OBCGP_mean+2.0*tf.sqrt(OBCGP_var)
...
for iter in range(50):
    ...
    acq_fcn_val = acq_fcn_UCB(tf.sigmoid(pt_sel), x_data_p, y_data_p, low_bdd_p,upp_bdd_p, X_M, phi_p, sigma_p,q_mu_p)
    ...
    print('iteration #:%d\t' % (iter + 1) + 'current Minimum value: %f\t' % (
    -np.max(Y)) + '\tcurrent Minimum point: (%f,%f)' % (X_M_best[0, 0], X_M_best[0, 1]))
